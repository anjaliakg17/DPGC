#Downloads\CODES\DPGCN\Privacy_Accuracy
import os
import torch
...
def hashed_values(data, no_of_hash, feature_size, function = 'dot'):
    Wl = torch.FloatTensor(no_of_hash, feature_size).normal_(0,1) #.uniform_(0,1) #.normal_(0,1
    Bin_values = torch.matmul(data, Wl.T)
    P_norm = max(np.linalg.norm(Wl.T, axis=0)) 
    return Bin_values, P_norm
#######################################################################################################################
...
#                                                DPGC                         #
#################################################################################################################
def DPGC(data, dataset, no_of_hash, binwidth, delta, epcilon):
    ...
    # Bin_values = hashed_values(wlx, no_of_hash, 2*feature_size, function = 'dot')
    
    Bin_values, max_norm = hashed_values(data.x, no_of_hash, feature_size, function = 'dot')
    sigma = max_norm*(np.sqrt(2*np.log(1/(2*delta))+ epcilon))/epcilon
    ...
    for bin_width in list_bin_width:
      ...
      #print(f"Features size for all of nodes: {np.shape(features)}")
    ...
    #print("g_coarse_dense size ",np.shape(g_coarse_dense))
...
binwidth = 0.05
for i in range(len(epcilon_list)):
    epcilon = epcilon_list[i]
    print("Epcilon:",  epcilon)
    for j in range(5):
